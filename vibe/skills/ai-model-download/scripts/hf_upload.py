    ...
    for attempt in range(max_retries):
        try:
            ...
            if "429" in err_str or "rate" in err_str or "too many" in err_str:
                ...
                logger.warning(
                    f"Upload error (attempt {attempt + 1}/{max_retries}): {e}, retrying in {wait}s..."
                )
                await asyncio.sleep(wait)
...
async def upload_file(
    filepath: Path, remote_path: str, config: HFConfig = None, commit_msg: str = None
) -> HFUploadResult:
...
async def upload_soul_snapshot(
    snapshot_path: Path, valence: float = 0.0, config: HFConfig = None
) -> HFUploadResult:
...
async def upload_semantic_cache(
    cache_path: Path, config: HFConfig = None
) -> HFUploadResult:
    """Upload the RLM semantic cache JSON."""
    return await upload_file(
        cache_path,
        "data/rlm_summary_cache.json",
        config,
...
async def append_to_jsonl(
    records: List[Dict], config: HFConfig = None
) -> HFUploadResult:
        ...
        for folder in ["lineage/.gitkeep", "data/.gitkeep", "metadata/.gitkeep"]:
            try:
                await asyncio.to_thread(
                    ...
                    repo_id=config.dataset_repo_id,
                    repo_type="dataset",
                    commit_message="Initialize folder structure",
                )
            except Exception:
                pass
