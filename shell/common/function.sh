...
# EasyTier status overview
et_status() {
    if ! command -v easytier-cli &>/dev/null; then
        echo "easytier-cli not found" && return 1
    fi
    echo "=== Node ==="
    easytier-cli node 2>/dev/null || true
    echo ""
    echo "=== Peers ==="
    easytier-cli peer 2>/dev/null || true
    echo ""
    echo "=== Routes ==="
    easytier-cli route 2>/dev/null || true
    echo ""
    echo "=== Proxy ==="
    easytier-cli proxy 2>/dev/null || true
    echo ""
}
